...
const { validateHabit, validateUUID } = require('../middleware/validate');
const db = require('../db');
...
router.post('/', authMiddleware, validateHabit, async (req, res, next) => {
    try {
        ...
        logger.info('Habit created:', { userId: req.user.id, habitId: id, name });
...
router.patch('/:id', authMiddleware, validateUUID, async (req, res, next) => {
    try {
        ...
        logger.info('Habit updated:', { userId: req.user.id, habitId: req.params.id });
...
router.delete('/:id', authMiddleware, validateUUID, async (req, res, next) => {
    try {
        ...
        logger.info('Habit deleted:', { userId: req.user.id, habitId: req.params.id });
...
router.post('/:id/toggle', authMiddleware, validateUUID, async (req, res, next) => {
    try {
        ...
        if (existing.rowCount > 0) {
            ...
            logger.info('Habit completion removed:', { userId: req.user.id, habitId: req.params.id, dateKey });
            ...
            logger.info('Habit completed:', { userId: req.user.id, habitId: req.params.id, dateKey });
            return res.json({ completed: true, date: dateKey, completionId: cid });
        }
    } catch (e) {
        logger.error('Error toggling habit:', e);
        next(e);
    }
});

router.get('/:id/stats', authMiddleware, validateUUID, async (req, res, next) => {
    try {
        ...
        logger.error('Error fetching habit stats:', e);
        next(e);
    }
});
...
router.get('/:id', authMiddleware, validateUUID, async (req, res, next) => {
    try {
        ...
        logger.error('Error fetching habit:', e);
        next(e);
    }
});

router.get('/:id/completions', authMiddleware, validateUUID, async (req, res, next) => {
    try {
        ...
        logger.error('Error fetching habit completions:', e);
        next(e);
    }
});

router.post('/:id/complete', authMiddleware, validateUUID, async (req, res, next) => {
    try {
        ...
        logger.info('Habit completed manually:', { userId: req.user.id, habitId: req.params.id, dateKey });
        res.status(201).json({ completion: { id: cid, timestampISO: ts.toISOString(), dateKey } });
    } catch (e) {
        logger.error('Error completing habit:', e);
        next(e);
    }
});

router.delete('/:id/completions/:completionId', authMiddleware, validateUUID, async (req, res, next) => {
    try {
        const del = await db.query('delete from habit_completions where id=$1', [req.params.completionId]);
        if (del.rowCount === 0) return res.status(404).json({ error: 'Not found' });
        logger.info('Habit completion deleted:', { userId: req.user.id, completionId: req.params.completionId });
        res.json({ success: true });
    } catch (e) {
        logger.error('Error deleting habit completion:', e);
        next(e);
    }
});

router.get('/summary/all', authMiddleware, async (req, res, next) => {
    try {
        ...
        logger.error('Error fetching habits summary:', e);
        next(e);
    }
});
